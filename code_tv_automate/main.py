...
def main(base_path, ppt_id, start_date, tv_data, num_days=10, study4=False):
    ...
    txt_path = os.path.join(base_path, ppt_id, '%s%s_data'%(ppt_id, str(dev_id).zfill(3)),'%s%s_data'%(ppt_id, str(dev_id).zfill(3)), 'txts')
    power_path = os.path.join(base_path, ppt_id, '%s%s_data'%(ppt_id, str(dev_id).zfill(3)),'%s%s_data'%(ppt_id, str(dev_id).zfill(3)), '%s%s_tv_power_5s.csv'%(ppt_id, str(dev_id).zfill(3)))
