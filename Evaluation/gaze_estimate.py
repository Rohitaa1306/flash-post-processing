...
def convert_lims(tv_data):
    ...
    loc_lims[:, 0] = slr * (loc_lims[:, 0]) - rls + lr_shift  # left right lower lim and upper lim
    loc_lims[:, 1] = slr * (loc_lims[:, 1]) + rls + lr_shift

    loc_lims[:, 2] = stb * (loc_lims[:, 2]) - tbs + tb_shift  # top bottom lower lim upper lim
...
def convert_to_gaze(at, loc_lims):
    ...
    for i in range(y.shape[0]):
        ...
        at[:, 3] = pd.to_numeric(at[:, 3], errors="coerce")
        at[:, 5] = pd.to_numeric(at[:, 5], errors="coerce")
